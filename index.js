	...
	*/
	var sayilar = [
	  ...
	  for (var i = 0; i < 1000; i++){
		var rastgeleSayi = Math.random() * 1000;
...
var ucetambolunenlerintoplami = ucetambolunenler.reduce(function(toplam, sayi){
	return toplam + sayi;
}, 0);
console.log("Üçe Tam Bölünenlerin Toplamı: ", ucetambolunenlerintoplami);
...
var siralisayilar = besyuzdenkucuksayilar.slice().sort(function(a, b){
	return a - b;
});
...
for (var i = 0; sayilar.length; i++){
	var sayilar = sayilar[i];
	var tekrarSayisi = 0;
}
for (var j = 0; sayilar.length; j++){
	if (sayilar[j] === sayi);
	tekrarSayisi++;
